...
def update_state(face, states, game_board, constraints):
    ...
    #Remove illegal states. (States claimed possible by face but not by surrounding faces)
    #Get possible southern states from northen tile (OR together all southern constraints for all possible states from northen tile)
    constraints_from_north = np.zeros_like(states[0])
    tile_idx = game_board[face][0]
    if(tile_idx==-1):
        constraints_from_north = np.ones_like(states[0])
    else:
        for i, s in enumerate(states[int(tile_idx)]):
            if(s):
                constraints_from_north=np.logical_or(constraints_from_north, constraints[i][num_tiles*2:num_tiles*3])

    constraints_from_east = np.zeros_like(states[0])
    tile_idx = game_board[face][1]
    if(tile_idx==-1):
        constraints_from_east = np.ones_like(states[0])
    else:
        for i, s in enumerate(states[int(tile_idx)]):
            if(s):
                constraints_from_east=np.logical_or(constraints_from_east, constraints[i][num_tiles*3:num_tiles*4])

    constraints_from_south = np.zeros_like(states[0])
    tile_idx = game_board[face][2]
    if(tile_idx==-1):
        constraints_from_south = np.ones_like(states[0])
    else:
        for i, s in enumerate(states[int(tile_idx)]):
            if(s):
                constraints_from_south=np.logical_or(constraints_from_south, constraints[i][num_tiles*0:num_tiles*1])

    constraints_from_west = np.zeros_like(states[0])
    tile_idx = game_board[face][3]
    if(tile_idx==-1):
        constraints_from_west = np.ones_like(states[0])
    else:
        for i, s in enumerate(states[int(tile_idx)]):
            if(s):
                constraints_from_west=np.logical_or(constraints_from_west, constraints[i][num_tiles*1:num_tiles*2])
    #Update possible states (AND together all constraints from all angles with current possible states)
    #pdb.set_trace()
    return states[face]*constraints_from_north*constraints_from_east*constraints_from_south*constraints_from_west
